...
const double Bhv_OmamMark::ATTACKER_DANGER_THRESHOLD = 0.20;
...
const int    Bhv_OmamMark::MAX_ASSIGNMENT_DIST = 50;      // max assignment dist (m)
...
Bhv_OmamMark::execute( PlayerAgent * agent )
{
    ...
    if ( ! target_opponent )
    {
        // No assignment found, but we're in marking zone - do active recovery
        // instead of returning false (which would fall to BasicMove)
        defensiveRecovery( agent );
        return true;
    }
...
Bhv_OmamMark::shouldMark( const PlayerAgent * agent )
{
    ...
    // Only mark when ball is in a zone where opponent pressing matters
    ...
    if ( ball_x > 33.0 )
    {
        // Ball deep in opponent territory - no need for tight marking
        return false;
    }
...
// ---------------------------------------------------------------------------
// Defensive Recovery (fallback when no assignment)
// ---------------------------------------------------------------------------

void
Bhv_OmamMark::defensiveRecovery( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    const ServerParam & sp = ServerParam::i();
    ...
    // Calculate a recovery position:
    // 1. On the line between ball and our goal
    // 2. Weighted more towards the ball when opponent has it
    // 3. Keep some defensive width

    Vector2D ball_to_goal = our_goal - ball_pos;
    double ball_to_goal_dist = ball_to_goal.length();

    if ( ball_to_goal_dist < 0.1 )
    {
        // Ball is at our goal position - just go to ball
        ball_to_goal_dist = 0.1;
    }

    Vector2D ball_to_goal_unit = ball_to_goal / ball_to_goal_dist;

    // Recovery distance from ball: between 3m and 10m towards our goal
    // Closer to ball when opponent has ball (opp_min is small)
    int opp_min = wm.interceptTable().opponentStep();
    double recovery_shift = std::min( 5.0 + opp_min * 0.5, 15.0 );

    Vector2D recovery_pos = ball_pos + ball_to_goal_unit * recovery_shift;

    // Add a lateral bias towards own position to avoid clustering
    Vector2D self_to_recovery = recovery_pos - self_pos;
    double self_to_recovery_dist = self_to_recovery.length();
    if ( self_to_recovery_dist > 2.0 )
    {
        Vector2D pullback = self_pos - recovery_pos;
        pullback.setLength( std::min( 3.0, self_to_recovery_dist * 0.3 ) );
        recovery_pos += pullback;
    }

    // Clamp to field
    double pitch_half_w = sp.pitchHalfWidth();
    double pitch_half_l = sp.pitchHalfLength();
    recovery_pos.x = std::max( -pitch_half_l + 1.0, std::min( pitch_half_l - 1.0, recovery_pos.x ) );
    recovery_pos.y = std::max( -pitch_half_w + 1.0, std::min( pitch_half_w - 1.0, recovery_pos.y ) );

    double dist_thr = wm.ball().distFromSelf() * 0.1;
    if ( dist_thr < 1.0 ) dist_thr = 1.0;

    dlog.addText( Logger::TEAM,
                  __FILE__": OMAM defensive recovery to (%.1f, %.1f)",
                  recovery_pos.x, recovery_pos.y );

    agent->debugClient().addMessage( "OMAM_Recovery" );
    agent->debugClient().setTarget( recovery_pos );

    double dash_power = Strategy::get_normal_dash_power( wm );

    if ( ! Body_GoToPoint( recovery_pos, dist_thr, dash_power
                           ).execute( agent ) )
    {
        Body_TurnToBall().execute( agent );
    }
