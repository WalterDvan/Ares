...
SamplePlayer::doPreprocess()
{
    ...
    //
    // predict teammate pass and move early
    //
    if ( doPredictedPassReceive() )
    {
        return true;
    }
...
*/
bool
SamplePlayer::doPredictedPassReceive()
{
    const WorldModel & wm = this->world();
    ...
    const std::vector< CooperativeAction::Ptr > & courses
        = StrictCheckPassGenerator::instance().courses( wm );

    CooperativeAction::Ptr best_course;
    double best_score = -1.0e10;

    for ( std::vector< CooperativeAction::Ptr >::const_iterator it = courses.begin();
          it != courses.end();
          ++it )
    {
        if ( ! (*it)
             || (*it)->category() != CooperativeAction::Pass
             || (*it)->targetPlayerUnum() != wm.self().unum()
             || (*it)->playerUnum() == wm.self().unum() )
        {
            continue;
        }

        const AbstractPlayerObject * passer = wm.ourPlayer( (*it)->playerUnum() );
        if ( ! passer
             || passer->isGhost()
             || passer->posCount() > 3 )
        {
        ...
        if ( passer->distFromBall() > wm.self().playerType().kickableArea() + 1.5
             && wm.interceptTable().teammateStep() > 2 )
        {
            continue;
        }

        const double dist_to_receive = wm.self().pos().dist( (*it)->targetPoint() );
        if ( dist_to_receive > 12.0 )
        {
            continue;
        }

        const double score
            = 20.0 - dist_to_receive
            + (*it)->targetPoint().x * 0.4
            - passer->pos().dist( (*it)->targetPoint() ) * 0.15;
    ...
    const Vector2D receive_point = best_course->targetPoint();
    const double dash_power = ServerParam::i().maxDashPower();
    const int self_min = wm.interceptTable().selfStep();

    dlog.addText( Logger::TEAM,
                  __FILE__": (doPredictedPassReceive) move early to (%.2f %.2f) from passer=%d",
                  receive_point.x, receive_point.y,
                  best_course->playerUnum() );

    this->debugClient().setTarget( receive_point );
    this->debugClient().addCircle( receive_point, 0.8 );
    this->debugClient().addMessage( "PredPassRecv" );

    if ( self_min <= 3
         && wm.ball().inertiaPoint( self_min ).dist( receive_point ) < 4.0 )
    {
        Body_Intercept().execute( this );
    }
    else
    {
        Body_GoToPoint( receive_point,
                        0.8,
                        dash_power ).execute( this );
    }

    this->setNeckAction( new Neck_TurnToBall() );
    this->setIntention( new IntentionReceive( receive_point,
                                              dash_power,
                                              0.9,
                                              3,
                                              wm.time() ) );
...
SamplePlayer::createActionGenerator() const
{
    ...
    //
    // multi action dribble at the root, then expand basic dribble in deeper layers
    //
    g->addGenerator( new ActGen_RangeActionChainLengthFilter
                     ( new ActGen_MultiActionDribble(), 1, 1 ) );
    g->addGenerator( new ActGen_RangeActionChainLengthFilter
